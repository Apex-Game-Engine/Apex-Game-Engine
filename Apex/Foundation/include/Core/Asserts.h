...
#define axCriticalAssert(condition)					do { if (!(condition)) { axCritical("Critial Assertion Failed : " STR(condition)); DEBUG_BREAK(); } else {} } while (false)
#define axCriticalAssertFmt(condition, fmt, ...)	do { if (!(condition)) { axCritical("Critical Assertion Failed : " STR(condition)); axCriticalFmt(fmt, ##__VA_ARGS__); DEBUG_BREAK(); } else {} } while (false)
...
#define axCheckFmt(condition, fmt, ...)
...
// Strong Asserts : Level 0 assertions : These are enabled in all builds, and cause SIGKILL in Release builds
#define axCriticalAssert(condition)					do { if (!(condition)) { axCritical("Critial Assertion Failed : " STR(condition)); DEBUG_BREAK(); } else {} } while (false)
#define axCriticalAssertFmt(condition, fmt, ...)	do { if (!(condition)) { axCritical("Critial Assertion Failed : " STR(condition)); axCriticalFmt(fmt, ##__VA_ARGS__); DEBUG_BREAK(); } else {} } while (false)
...
#define axAssertFmt(condition, fmt, ...)

// Strong Asserts : Level 0 assertions : These are enabled in all builds, and cause SIGKILL in Release builds
#define axCriticalAssert(condition)					do { if (!(condition)) { KILL(); } else {} } while (false)
#define axCriticalAssertFmt(condition, fmt, ...)	do { if (!(condition)) { KILL(); } else {} } while (false)

// Verifies : Level 0 assertions : These are enabled in all builds, but do not raise errors in Release builds
#define axVerify(condition)							(condition)
#define axVerifyFmt(condition, fmt, ...)			(condition)
