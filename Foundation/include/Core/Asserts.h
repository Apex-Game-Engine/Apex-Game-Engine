...
#define axCheckMsg(condition, fmt, ...) do { if (!(condition)) { axError("Check Failed! : " XSTR(condition)); axErrorFmt(fmt, ##__VA_ARGS__); DEBUG_BREAK(); } else {} } while (false)
...
#define axStrongAssertMsg(condition, fmt, ...) do { if (!(condition)) { axError("Strong Assertion Failed : " XSTR(condition)); axErrorFmt(fmt, ##__VA_ARGS__); DEBUG_BREAK(); } else {} } while (false)
...
#define axVerifyMsg(condition, fmt, ...) do { if (!(condition)) { axError("Verify Failed! : " XSTR(condition) ""); axErrorFmt(fmt, ##__VA_ARGS__); DEBUG_BREAK(); } else {} } while (false)
