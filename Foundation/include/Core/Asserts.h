...
#define axCheckMsg(condition, msg) do { if (!(condition)) { axError("Check Failed! : " XSTR(condition)); axError(msg); DEBUG_BREAK(); } else {} } while (false)
...
#define axVerifyMsg(condition, msg) do { if (!(condition)) { axError("Verify Failed! : " XSTR(condition) ""); axError(msg); DEBUG_BREAK(); } else {} } while (false)
